...
pub async fn send_notify(pool: &Arc<Mutex<Pool>>, ctx: &serenity::Context) {
    ...
    let contests: Vec<&Contest> = contests
        ...
        .filter(|contest| {
            ...
            start_time.date_naive() <= chrono::Local::now().date_naive()
                && end_time.date_naive() >= chrono::Local::now().date_naive()
        })
        .collect();
        ...
        let response = {
            ...
            for contest in contests {
                ...
                let embed = CreateEmbed::new()
                    ...
                    .field(
                        "開催時間",
                        format!("<t:{0}:f>(<t:{0}:R>)", start_time.timestamp()),
                        false,
                    )
                    .field(
                        "終了時間",
                        format!("<t:{0}:f>(<t:{0}:R>)", end_time.timestamp()),
                        false,
                    )
                    .field(
                        "Rated対象",
                        format!("`{}`", contest.rating_range_raw),
                        false,
                    );
            ...
            message
                .content("今日のコンテストです")
                .add_embeds(embed_vec)
        };
        let channels: Vec<String> = conn
            .query("SELECT contest_channel_id FROM notifications")
            .unwrap();
        for i in channels {
            log::info!("{}", i);
            if i != "null" {
                ...
                let temp = channel
                    .send_message(ctx.http.clone(), response.clone())
                    .await;
                match temp {
                    Ok(t) => {
                        println!("{:?}", t);
